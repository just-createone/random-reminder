...
修改 Docker 运行用户、UID/GID 或持久化目录权限时，还必须验证旧版本升级场景。旧 root 版本迁移到非 root 镜像时，应使用 `scripts/migrate_runtime_permissions.py` 处理运行数据 ownership，不能通过把数据库长期改成 `777` 来规避权限问题。
...
```text
完成 v0.1.5 权限迁移修复与正式发布
→ 完成 v0.1.6 production 测试推送路由修复与发布准备
→ 发布并验证 v0.1.6 正式镜像
→ 完成 PWA 独立窗口最终验收
→ 完成 Git 完整历史敏感信息检查
→ 启动首批真实用户测试
