# 版本变更记录

本文档记录随机提醒器各版本的重要功能、修复和工程改进。

版本格式：

```text
主版本.次版本.修复版本
```

例如：

```text
0.1.6
```

版本类型：

- 主版本：存在重大功能或不兼容变化
- 次版本：增加一组新功能
- 修复版本：修复问题或完成小范围改进

---

## [Unreleased]

### 新增

- v0.3.0 开发中：新增邮箱/密码注册、登录、退出登录与安全会话
- v0.3.0 开发中：提醒、设置、计划、通知历史、导入导出和 Web Push 改为按用户隔离
- v0.3.0 开发中：新增用户时区，计划生成与后台执行器按用户当地时间运行
- 新增用户数据导出后端 API：`GET /api/data/export`
- 新增用户数据导入后端 API：`POST /api/data/import`
- 新增 data transfer service 与 repository，用于导出数据和原子导入数据
- FastAPI 应用注册 data router

### 测试

- 新增真实 HTTP 会话隔离测试，验证匿名访问被拒绝、不同账号无法读取对方数据
- 新增多用户计划、通知历史、Web Push 与时区执行隔离测试
- 新增时区感知计划生成回归测试
- v0.3.0 本机人工验收已完成：双账号数据隔离、设置隔离、导出隔离、退出登录与计划生成通过
- v0.3.0 人工验收后完整 Pytest 回归通过
- 新增数据导入/导出自动化测试
- 路由注册测试改为根据 OpenAPI paths 验证最终公开 API
- Task 4A 专项测试：`35 passed in 3.52s`
- 当前开发分支完整回归：`108 passed in 10.64s`

### 计划新增

- 启动首批真实用户测试

---

## [0.1.7] - 2026-08-09

### 修复

- 修复设置页缺少 `ui.js` 导致保存成功/失败提示异常的问题
- 清理首页重复的浏览器通知和 Web Push 控件以及错误 HTML 结构
- 移除失效且未被产品使用的 executor HTTP 调试 API

### 测试

- 增加设置页脚本依赖顺序回归测试
- 增加首页通知控件 ID 唯一性回归测试
- 增加 executor API 不再注册和不出现在 OpenAPI 中的回归测试

---

## [0.1.6] - 2026-08-09

### 修复

- 修复 production 环境虽然从 OpenAPI 隐藏 `/api/push/test-send`，但路由仍被 FastAPI 注册的问题
- `DEBUG=False` 时不再注册 `/api/push/test-send` 的 `APIRoute`，因此请求不会进入测试推送接口的请求体校验或业务处理
- `DEBUG=True` 时继续注册测试推送接口，保留开发环境调试能力
- 保留函数内部的 production `404` 防护，作为直接函数调用时的额外保护

### 测试

- 新增 production 环境“测试推送路由未注册”回归测试
- `tests/test_push_openapi.py` 由 `3` 项增加到 `4` 项并全部通过
- 完整 Pytest 回归由 `63` 项增加到 `64` 项并全部通过
- `v0.1.5` 正式环境 Smoke Test 曾复现无请求体 POST `/api/push/test-send` 返回 `422`；本版本通过取消 production 路由注册修复该问题

## [0.1.5] - 2026-08-09

### 修复

- 修复旧版 root 容器直接升级到非 root 镜像时，`random_reminder.db` 仍为 `root:root / 0644`，导致 UID `10001` 启动时报 `sqlite3.OperationalError: attempt to write a readonly database` 的升级兼容问题
- 新增一次性权限迁移脚本 `scripts/migrate_runtime_permissions.py`
- 权限迁移脚本处理 `/app/data`、主数据库及可能存在的 `-wal`、`-shm`、`-journal` 文件，以及 `/app/backups` 目录
- 权限迁移只调整目标对象的 UID/GID，不修改数据库内容，不放宽文件 mode，也不修改 VAPID 密钥
- 非 root 正式应用容器继续保持 UID/GID `10001` 运行，无需以 root 身份常驻

### 升级与部署

- 从 `v0.1.3` 或更早的 root 运行版本升级时，支持使用新镜像以一次性 root 容器执行权限迁移
- 支持 `--dry-run` 预览准备迁移的路径
- 迁移完成后，UID `10001` 可以正常写入原 SQLite 数据库并创建数据库备份
- 如果某个 `v0.1.4` 部署已经手动完成数据库 ownership 迁移并确认可写，可以直接升级；重复迁移到相同 UID/GID 也不会修改数据库内容

### 测试

- 新增 `5` 项权限迁移自动化测试
- 完整 Pytest 回归由 `58` 项增加到 `63` 项并全部通过
- 使用 Docker named volume 真实复现 `root:root / 0644` 数据库场景
- 验证迁移前 UID `10001` 写入失败，错误与正式升级时复现的 readonly database 一致
- 验证迁移后数据库与运行目录变为 UID/GID `10001`，原数据保持不变且可以继续写入
- 验证完整 FastAPI 应用可以使用迁移后的旧数据库正常启动
- 验证迁移后数据库完整性、应用初始化和数据库备份功能正常

---

## [0.1.4] - 2026-08-08

### 修复

- 修复备份清理模块仍使用旧 `BACKUP_RETENTION_COUNT` 的问题
- 备份清理正式使用 `RANDOM_REMINDER_BACKUP_KEEP_LATEST` 和 `RANDOM_REMINDER_BACKUP_MAX_AGE_DAYS`
- 最新保留范围内的备份始终受到保护，保护范围之外仅删除超过最大保存天数的标准备份
- 非标准时间戳备份文件默认保留，避免误删
- production 环境中的 `/api/push/test-send` 不再出现在 OpenAPI 文档中，并保留 handler 级 `404` 防护
- 该版本仍会注册测试推送路由；无请求体请求可能在进入 handler 前被校验为 `422`，路由注册问题在 `v0.1.6` 修复

### Docker 与部署

- Docker 镜像增加专用 `app` 用户和组，运行时 UID/GID 为 `10001`
- 正式应用不再以 root 用户运行
- Release 镜像加入 `scripts/generate_vapid_keys.py`
- VAPID 生成脚本支持在镜像中直接生成三项密钥文件，并拒绝覆盖已有密钥
- 继续保持 `/app/secrets/vapid` 正式运行时只读挂载
- 验证 `/app/data` 和 `/app/backups` 在非 root 用户下仍可写

### 测试

- 增加 production / development OpenAPI 行为测试
- 增加备份最大保存天数、边界时间、最新备份保护、异常文件名保护等测试
- 完整 Pytest 回归达到 `58` 项并全部通过
- 验证备份创建、恢复、损坏备份拒绝、安全备份和清理 CLI
- 验证容器异常退出恢复、容器删除重建后的数据持久化
- 验证 VAPID 密钥跨容器重建保持不变且挂载只读
- 验证 Push 订阅跨容器重建持久化
- 验证 Android Chrome 后台、锁屏 Web Push 和通知点击跳转
- 验证 AMD64 与 ARM64 多架构镜像清单
- 按项目验收口径完成二十四小时稳定性验证

### 安全

- 验证 `.env.release`、数据库、备份和 VAPID 密钥未被 Git 跟踪
- 验证 Release 镜像本身不包含 `.env.release`、数据库或 VAPID 私钥
- 验证运行日志未发现 VAPID 私钥、Push auth key 或 Authorization Header 泄露
- 验证容器未启用 privileged 模式、未挂载 Docker Socket、未额外添加 Linux capability

---

## [0.1.3]

### 新增

- 增加数据库旧备份清理功能
- 支持按照最新备份数量保留备份
- 支持按照最大保存天数清理备份
- 增加旧备份清理预览模式
- 增加备份清理命令行入口
- 增加备份清理自动化测试

### 配置

增加以下环境变量：

```text
RANDOM_REMINDER_BACKUP_KEEP_LATEST
RANDOM_REMINDER_BACKUP_MAX_AGE_DAYS
```

正式环境默认策略：

```text
保留最新 30 份备份
最大保存时间 90 天
```

### 工程改进

- 数据库备份完成后自动执行旧备份清理
- 完善 Docker Compose 备份配置
- 更新环境变量示例文件
- 发布 AMD64 和 ARM64 正式 Docker 镜像

---

## [0.1.2]

### 新增

- 增加 SQLite 数据库恢复命令
- 支持从指定备份恢复数据库
- 支持恢复前自动创建安全备份
- 支持关闭恢复前安全备份
- 支持指定恢复目标数据库
- 支持指定安全备份目录
- 增加 SQLite 完整性检查
- 增加损坏备份拒绝机制
- 增加数据库恢复自动化测试

### 安全改进

- 恢复前验证备份数据库完整性
- 恢复后验证临时数据库完整性
- 避免恢复失败破坏当前数据库
- 数据库被占用时显示明确错误

### 工程改进

- 发布包含数据库恢复能力的正式 Docker 镜像
- 完善容器内数据库恢复流程

---

## [0.1.1]

### 新增

- 增加 SQLite 数据库备份命令
- 支持指定数据库来源
- 支持指定备份目录
- 支持自动生成带时间戳的备份文件名
- 增加数据库备份自动化测试

### 修复

- 修复 Windows 下 SQLite 连接未及时关闭导致的文件占用问题
- 使用 SQLite Backup API 创建一致性备份
- 使用临时文件和原子替换降低损坏风险

### 配置

增加环境变量：

```text
RANDOM_REMINDER_BACKUP_DIR
```

### Docker

- 将宿主机 `backups/` 目录挂载到容器 `/app/backups`
- 确保数据库备份不会因容器删除而丢失
- 排除备份文件进入 Git 和 Docker 构建上下文

---

## [0.1.0]

### 核心功能

- 创建提醒内容
- 查看提醒列表
- 删除提醒内容
- 设置提醒总开关
- 支持全天提醒
- 支持指定提醒时间范围
- 自动生成每日随机提醒计划
- 支持强制重新生成今日计划
- 显示下一次提醒时间
- 显示提醒倒计时
- 自动跳过过期提醒

### 通知

- 支持 Windows 本地系统通知
- 支持控制台通知回退
- 支持浏览器通知
- 支持 Web Push
- 支持 VAPID 密钥
- 支持 Service Worker 接收推送
- 增加测试通知接口

### 数据

- 使用 SQLite 保存提醒内容
- 保存提醒设置
- 保存每日提醒计划
- 保存通知执行记录
- 支持应用重启后数据保留

### PWA

- 增加 Web App Manifest
- 增加 Service Worker
- 增加 192×192 应用图标
- 增加 512×512 应用图标
- 支持安装到桌面
- 支持独立窗口运行

### 后端

- 使用 FastAPI
- 使用统一配置模块
- 使用应用生命周期管理数据库和执行器
- 增加健康检查接口
- 增加 API 文档
- 增加后台提醒执行器

### 前端

- 增加首页
- 增加提醒管理页面
- 增加设置页面
- 增加全局成功和错误提示
- 增加自定义删除确认框
- 增加响应式页面布局

### 测试和工程化

- 增加 Pytest 自动化测试
- 增加 Windows GitHub Actions 测试
- 增加 Ubuntu GitHub Actions 测试
- 增加 Docker AMD64 构建
- 增加 Docker ARM64 构建
- 增加 Git 标签发布流程
- 将镜像发布到 GitHub Container Registry

### 部署

- 增加 Dockerfile
- 增加本地 Docker Compose 配置
- 增加正式镜像部署配置
- 支持环境变量配置
- 支持宿主机数据库目录挂载
- 支持 VAPID 密钥只读挂载
- 支持容器健康检查
