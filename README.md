...
随机提醒器是一个基于 FastAPI、SQLite 和 PWA 构建的随机提醒应用。

用户可以维护自己的提醒文本，系统每天随机生成提醒计划，并在指定时间发送浏览器推送或本地系统通知。

当前版本：`v0.1.7`

当前开发阶段：`v0.3.0` 多用户公开测试版本开发中，尚未正式发布。

`v0.3.0` 已完成账号注册与登录、用户数据隔离、按用户时区生成计划，以及用户专属 Web Push 的基础能力。当前仍处于开发和测试阶段，不代表 `v0.1.7` 已包含这些功能。
...
- 邮箱和密码注册、登录、退出登录
- 登录会话保护，未登录用户不能访问业务数据
- 提醒、设置、计划、通知历史、导入导出按用户隔离
- 按用户注册时区生成和执行当天提醒计划
- Web Push 订阅与提醒推送按用户隔离
...
最新开发分支验证（`v0.2.0` Task 4A，不属于 `v0.1.7` 发布记录）：

- `python -m pytest tests/test_data_transfer.py -v`：`35 passed in 3.52s`
- `python -m pytest -v`：`108 passed in 10.64s`
...
`v0.2.0` 已完成用户数据导入/导出的最小闭环：

- `GET /api/data/export`：导出可携带的提醒与设置数据。
- `POST /api/data/import`：导入并校验可携带的提醒与设置数据。
- 上述接口返回现有的 `success`、`data`、`message` 响应结构。
- Task 4B 已在设置页提供导出、选择 JSON 文件和确认导入入口。
- 导入会保留当前提醒、追加文件中的提醒，并恢复文件中的提醒设置。
- 该能力已通过自动化测试和隔离数据库浏览器验收；`v0.2.0` 仍是公开测试候选，尚未正式发布。

---

## 后续计划

- 完成 Task 5 公开测试文档与候选版本收口
- 推送并部署 `v0.2.0` 公开测试候选（不等同于正式发布）
- 准备 5 至 10 人首批真实用户测试
- 根据真实用户反馈决定后续功能范围
